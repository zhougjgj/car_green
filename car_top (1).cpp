...
using vehicle_cmd_msg_t = car_top::VehicleCmd;
...
void cmd_velCallback(const car_top::VehicleCmd::twist_cmd &twist_aux)//速度控制回调
{
...
int main(int argc, char **argv)
{
    ...
    config.Timing0=0x01;/*波特率500 Kbps  0x00  0x1C*/
    config.Timing1=0x1C;
    config.Mode=0;//正常模式
    ...
    */
    ...
    while (ros::ok())
    {
		...
		else if(rev[i].ID == 0x1D1)//can chao sheng bo
                {
                   int data_1=rev[i].Data[0] ;
                   int data_2=rev[i].Data[1] ;
                   int data_3=rev[i].Data[2] ;
                   int data_4=rev[i].Data[3] ;
                   int data_5=rev[i].Data[4] ;
                   int data_6=rev[i].Data[5] ;
                   int data_7=rev[i].Data[6] ;
                   int data_8=rev[i].Data[7] ;
                    cout << "  1hao=="<<data_1<<"  2=="<<data_2<<"  3=="<<data_3<<"  4=="<<data_4
                    <<"  5=="<<data_5<<"  6=="<<data_6<<"   7=="<<data_7<<"  8=="<<data_8<< endl;
